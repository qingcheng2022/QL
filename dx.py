...
run_num=os.environ.get('reqNUM') or "10"
...
def load_exchange_records():
    try:
        record_file = 'exchange_records.json'
        ...
        if os.path.exists(record_file):
            with open(record_file, 'r') as f:
                records = json.load(f)
                # 只保留本月的记录
                records = {k: v for k, v in records.items() if v.split(' ')[0].startswith(current_month)}
                return records
        return {}
...
def save_exchange_record(phone):
    try:
        record_file = 'exchange_records.json'
        records = load_exchange_records()
        records[phone] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
def run_Time(hour,miute,second):
    date = datetime.now()
    date_zero = datetime.now().replace(year=date.year, month=date.month, day=date.day, hour=hour, minute=miute, second=second)
...
async def qgNight(phone, ticket, timeDiff,isTrue):
    ...
    if runTime >(time.time()+timeDiff):
        ...
        print(f"当前时间:{str(datetime.now())[11:23]},跟设定的时间不同,等待{difftime}秒开始兑换每天一次的")
    ...
    if sign:
        # print(f"当前时间:{str(datetime.now())[11:23]}获取到了Sign:"+sign)
...
async def main(timeDiff,isTRUE,hour):
    ...
    for phoneV in phone_list:
        ...
        # 检查是否在本月已经兑换过
        if phone in records:
            exchange_time = datetime.strptime(records[phone], '%Y-%m-%d %H:%M:%S')
            if exchange_time.strftime('%Y-%m') == current_month:
                printn(f"{get_first_three(phone)}: 本月已兑换成功（{records[phone]}），跳过该账号")
                continue
