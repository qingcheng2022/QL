...
定时: 1 56 23 * * *
变量格式:
账号 chinaTelecomAccount = 123123123123@26666  多号&隔开
口令 dx_kl = 绑定福利,绑定福利2
'''
...
def ttt(hour, minute, second):
    ...
    date_zero = datetime.datetime.now().replace(year=date.year, month=date.month, day=date.day, hour=hour,
...
@retry(
    ...
    before_sleep=lambda retry_state: sync_before_sleep(retry_state, retry_state.args[0], retry_state.args[1],
                                                       retry_state.args[2])
)
...
async def getSign(ticket, SS, ckvalue):
    try:
        response_data = await SS.get('https://wapside.189.cn:9001/jt-sign/ssoHomLogin?ticket=' + ticket,
                                     cookies=ckvalue)
                    ...
                    if response.status_code == 200:
                        ...
                        if login['success'] == True:
                            ...
                            if useridv:
                                for code in WELFARE_CODES:
                                    ...
                                    response = await s.post('https://wapact.189.cn:9001/yzf1/welfare/convert',
                                                            data=json.dumps(convertpayload))
                                    if response.status_code == 200:
                        ...
                        if response.status_code == 200:
                            ...
                            if datavv.get('success') != True:
                                print("领取异常：", datavv)
                                return
                            if datavv.get('result') is not None and len(datavv['result']) > 0 and datavv['result'][
                                0] is not None:
...
WELFARE_CODE = os.environ.get('dx_kl') or "绑定福利,事事如意,2025加油"
